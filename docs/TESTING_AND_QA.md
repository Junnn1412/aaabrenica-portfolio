...
| Pure-logic unit tests                  | Node built-in `node:test`                    | `npm test`                               | `escapeHtml`, marker counting/composition, `validateContent`, `isSafeInternalPath`, `normalizePath`/`resolveEntryPath`, `renderRoute` integration, a `validate-routes.mjs` exit-code smoke test, the preview composer's exact-file boundary (`resolveHtmlRequest`), 25 design-token WCAG contrast pairs (TEXT/LARGE-TEXT-UI/NON-TEXT/FOCUS-INDICATOR, via distinctly-named threshold constants) compiled from the real `_custom-properties.scss`, and (PF-030) the showcase's table-of-contents/heading invariants read from the real committed `dev/design-system/index.html` — every `href="#..."` resolves to a real `id`, every `id` is unique, and exactly one `<h1 id="page-title">` exists |
