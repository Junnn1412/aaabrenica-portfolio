...
| Pure-logic unit tests                  | Node built-in `node:test`                    | `npm test`                               | `escapeHtml`, marker counting/composition, `validateContent`, `isSafeInternalPath`, `normalizePath`/`resolveEntryPath`, `renderRoute` integration, a `validate-routes.mjs` exit-code smoke test, the preview composer's exact-file boundary (`resolveHtmlRequest`), and design-token WCAG contrast ratios compiled from the real `_custom-properties.scss` |
