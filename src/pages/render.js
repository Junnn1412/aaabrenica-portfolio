...
export function renderRoute(route) {
  ...
  if (!content) {
    throw new Error(`[render] missing content module for route "${route.key}" (expected key "${route.content}")`);
  }
  ...
  if (!renderTemplate) {
    throw new Error(`[render] unknown template "${route.template}" for route "${route.key}"`);
  }

  const { header, main, footer } = renderTemplate({ content, navItems: primaryNav, activeKey: route.navKey });
